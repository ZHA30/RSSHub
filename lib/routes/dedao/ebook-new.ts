...
const buildDescription = (payload?: DetailPayload): string | undefined => {
    ...
    if (details.length) {
        blocks.push('<h3>书籍信息</h3>', `<ul>${details.join('')}</ul>`);
    }

    if (detailData.bookIntro) {
        blocks.push('<h3>内容简介</h3>', `<p>${formatText(detailData.bookIntro)}</p>`);
    }

    if (detailData.press?.brief) {
        blocks.push('<h3>出版社简介</h3>', `<p>${formatText(detailData.press.brief)}</p>`);
    }
