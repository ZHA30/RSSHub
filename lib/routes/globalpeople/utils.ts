...
const requestUserAgent = config.trueUA;
...
const getCookiePair = (cookie: string) => cookie.split(';')[0]?.trim() ?? '';

const mergeCookies = (...cookieLists: string[][]) => {
...
const extractDocumentCookies = (html: string) => [...html.matchAll(/document\.cookie\s*=\s*["']([^"']+)["']/g)].map((match) => getCookiePair(match[1])).filter(Boolean);
...
const getFreshCookie = async (url: string) => {
    ...
    const headerCookies = response.headers.getSetCookie().map((setCookie) => getCookiePair(setCookie));
    const documentCookies = extractDocumentCookies(response._data ?? '');
    const cookie = mergeCookies(headerCookies, documentCookies);
...
export async function handleSection(id: string): Promise<Data> {
    ...
    let items = getListItems($, currentUrl, defaultLimit);
