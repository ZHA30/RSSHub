...
const buildRequestHeaders = (rootUrl: string) => ({
...
const fetchPage = (url: string, rootUrl: string) =>
    ofetch<string, 'text'>(url, {
        responseType: 'text',
        headers: buildRequestHeaders(rootUrl),
    });
...
const buildIssueUrl = (rootUrl: string, paperId: string, issue: string) => `${rootUrl}/${paperId}/mobile/${issue.slice(0, 4)}/${issue}/${issue}_m.html`;
...
const getIssueContext = async (rootUrl: string, paperId: string, paperName: string, rootPageUrl: string, issue?: string) => {
    if (issue) {
        const issueUrl = buildIssueUrl(rootUrl, paperId, issue);

        try {
            const issueHtml = await cache.tryGet(issueUrl, () => fetchPage(issueUrl, rootUrl));
    ...
    const rootHtml = await cache.tryGet(rootPageUrl, () => fetchPage(rootPageUrl, rootUrl));
    const latestIssue = extractLatestIssue(rootHtml, paperName, rootPageUrl);
    const issueHtml = await cache.tryGet(latestIssue.issueUrl, () => fetchPage(latestIssue.issueUrl, rootUrl));
...
export const createNewspaperRoute = ({ paperId, paperName, host = 'bjrbdzb.bjd.com.cn' }: NewspaperConfig): Route => {
    ...
    const handler: Route['handler'] = async (ctx) => {
        ...
        const { issue: resolvedIssue, issueUrl, issueHtml } = await getIssueContext(rootUrl, paperId, paperName, rootPageUrl, issue);
            ...
            async (article) => {
                try {
                    return await cache.tryGet(article.link, async () => {
                        const detailHtml = await fetchPage(article.link, rootUrl);
                    ...
                    return {
                        ...
                        link: article.link,
                        pubDate: timezone(parseDate(formatIssueDate(article.issue), 'YYYY-MM-DD'), +8),
    ...
    return {
        ...
        radar: [
            {
                source: [`${host}/${paperId}/paperindex.htm`],
                target: `/${paperId}`,
            },
        ],
        name: routeName,
        maintainers: ['ZHA30'],
        handler,
        url: `${host}/${paperId}/paperindex.htm`,
